...
#The index postion of the element in the vector matches the networkx label of the node, so 0th element of vector is the 0th labelled node. 
def HierarchicalLevels(graph):
    A = nx.adjacency_matrix(graph).transpose()
    k_in = np.asarray(A.sum(axis=1))
    D_in = np.diagflat(k_in)
    L_in = D_in - A    
    return np.linalg.lstsq(L_in, k_in)[0]
...
# Just need to create sparse versions for the above function
